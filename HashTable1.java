...
public class HashTable1{
	...
	String [] hshs = new String[TableSize];
	...
	*/
	public int hash(String s){
		//System.out.println("calling hash : ~ "+ s +" | " + s.charAt(0));
		//simple hashers function , 
		int sum =0 ; 

		for(int i =0 ; i<s.length() ; i++){
			sum += Math.pow((int)s.charAt(i),i);
		}
	...
	// 		sum +=  Math.pow(3,charVal)* charVal;
	...
	*/
	public int insert(String s){
		...
		while(T.getElement(hi) != null && hshs[hi] !=null  ){
		...
		System.out.println("Inserted :" + s + "  at hshs["+ hi+"]");
		return hi;
	}
	...
	*/
	public int find(String s){
		...
		//Get the hash value of the string and start the search at that index.
		int hi = hash(s);
		System.out.println("find on : "+s  +".    hashed at ix : " + hi );
		...
		// 	if (s.equals(element) && s == item)
		// 		return i;
		// 	//If the string was not at this index, continue looking.
		// 	if(s == item){
		// 		System.out.println("found item : " +s +" at index; " + i );
		// 		return i;
		// 	}
