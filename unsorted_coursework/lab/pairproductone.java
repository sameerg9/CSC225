...
public class pairproductone{
	...
	*/
	public static boolean PairSum225(int[] A){
		...
		for(int i = 0 ; i<n ; i++){
			int j = A[i]; 
			if( j >=0 &&  j <= 225){
				t[j]++; //increment aux value for considered innputs
				System.out.println("incremented t[j] at ix:"+ i+ "     |   arrValue = "+ j);
			} 
		}
		...
		for(int j = 0 ;j <=225 ; j++){
			...
			System.out.println("t[j]:"+t[j]+"	 t[225-j] : "+t[225-j] + "    j_inter:"+j);
			if(t[j] > 0 && t[225%j]== 0 ){
				System.out.println("\npair found\n ");
	...
	*/
	public static void main(String[] args){
		...
		System.out.printf("Array %s a pair of values whose product is 225.\n",pairExists? "contains":"does not contain");
