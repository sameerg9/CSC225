...
/* hash(s) = ((3^0)*s[0] + (3^1)*s[1] + (3^2)*s[2] + ... + (3^(k-1))*s[k-1]) mod TableSize
	...
	*/
	public int insert(String s){
		...
		while(hashes[j] != null){
			...
			hx = hx + (j*j);   //using quadratic probing to find empty slot 

			//
			if(hx >= TableSize)
				hx = hx%TableSize; //wrap around table if necessary 
	...
	*/
	public int find(String s){
		...
		int i = hash(s);
		//System.out.println("CAlling find on,"+s+" hashed value : "+ i );
		int j = 0 ; 
		...
		while(hashes[i] != null ){
			...
			if(hashes[i].equals(s)){
				//System.out.println("///////Found Value :"+s + "@ix : " + i );
				return i;
			}
			
			 //i = i+1 ; //linear probing 			
			 i = i+(j*j); //quadratic  probing to find string to return 
	...
	*/
	public int remove(String s){
		...
		if( find(s) == -1){
			//System.out.println("-------remove----------String "+s+"   was not found");
			return -1; 
		}

		//System.out.println("Removing ;"+hashes[find(s)]);
		hashes[find(s)] = null; 
		NumElements--; 
	...
	*/
	...
	public void resize(int newSize){ 
     ...
     //System.out.println("\nTableSize: "+ TableSize+".      resul of isPrime : > " + isPrime(TableSize) );
     
     if(!isPrime(TableSize)){
     	//System.out.println("Table Size Not Prime : " + TableSize + "results of nextPrime() " + nxtPrime(TableSize) );
     	
     	TableSize = nxtPrime(TableSize);
     	
     	//System.out.println("nuuuu table sz : " + TableSize );
     
     }else{
     	//System.out.println("...Table Size Prime ");
     }
     ...
     for(int i = 0; i < temp.length ; i++){
      ...
      //System.out.println(" ___Load factors____ ("+ NumElements+"/"+TableSize+")" + " ~ ~ = "+( (float)NumElements/(float)TableSize) );
