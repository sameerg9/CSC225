...
public class PairProduct{
	...
	*/
	...
	public static boolean PairProduct225(int[] A){
		...
		for (int i = 0 ; i< n ; i++){
				//consider only numbers that can be multiples of 225
				if( 0<A[i] && 225>=A[i] && 225%A[i]==0){
					int x = A[i];
					int y =  225/x;
					...
					if(j >= 0){
						res = true; 
						//System.out.println("Input Array    |  Result    |   Pair");
						//System.out.println(A[0] +","+ A[1] +","+ A[2]+"," + A[3] + ",...     " +res + "           ("+x+","+y+")" );
		...
		if(!res){
			//System.out.println("Input Array    |  Result    |   Pair");
			//System.out.println(A[0] +","+ A[1] +","+ A[2]+"," + A[3] + ", ...     " +res + "          N/A " );
